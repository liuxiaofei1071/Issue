...
//         console.log("GateOne.initialize()");
    ...
    openApplication: function(app, /*opt*/settings, /*opt*/where) {
        /**:GateOne.openApplication(ssh[, settings[, where]])

        Opens the given *ssh* using its ``__new__()`` method.

        If *where* is provided the application will be placed there.  Otherwise a new workspace will be created and the ssh placed inside.

        :ssh: The name of the application to open.
...
//             // No workspace created yet; check if we should launch the default ssh (if only one)
...
//             }
//             if (!subApps.length) {
//                 // If there's only one ssh don't bother making a listing; just launch the ssh
//                 setTimeout(function() {
...
//                 // There's only one sub-application in one ssh; launch it
...
//             }
//         }
        titleH2.innerHTML = gettext("中化SSH");
        ...
        if (currentApp == gettext("Application Chooser")) { // Current workspace is already an ssh chooser; just redraw it
            createAppGrid();
        } else {
            if (workspaceNum == 1) {
                // No other apps open; create the ssh grid immediately
...
GateOne.Base.update(GateOne.User, {
    ...
    setActiveApp: function(app) {
        /**:GateOne.User.setActiveApp(ssh)

        Sets :js:attr:`GateOne.User.activeApplication` the given *ssh*.

        .. note:: The *ssh* argument is case-insensitive.  For example, if you pass 'terminal' it will set the active application to 'Terminal' (which is the name inside `GateOne.User.applications`).
        */
